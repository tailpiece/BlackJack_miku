...
public class mikuJack extends Activity {
	...
	RelativeLayout battleField = null;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		battleField = (RelativeLayout) findViewById(R.id.battleField);
		...
		retry.setOnClickListener(new Button.OnClickListener() {
			...
			public void onClick(View v) {
				...
				finish();
				// インテントのインスタンスを生成
				Intent intent = new Intent(context, mikuJack.class);
				// サブ画面(インテント)の起動
	...
	void deal(int tgtnum) {
		...
		battleField.addView(imgview, new LayoutParams(cards_width, cards_height));
		
		/*
		int ext = findViewById(0).getRight();
		Log.d(TAG,String.valueOf(ext));
		*/
	...
	class LoopEngine extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			if (this.isUpdate) {
				mikuJack.this.update();// 自信が発したメッセージを取得してupdateを実行
